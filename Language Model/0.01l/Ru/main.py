...
while True:
    ...
    if usersChoice == '1':
        model = load_model('PhoneReviewRu-Lang')
        while True:
            userInput = input('Prompt: ')
            for i in range(50):
                userInput = userInput, token.index_word[np.argmax(model.predict([token.texts_to_sequences([userInput.split(' ')[-3], userInput.split(' ')[-2], userInput.split(' ')[-1]])]))+1]
        ...
        model = Sequential()
        model.add(Embedding(input_dim=21362, output_dim=150))
        model.add(GRU(256))
        model.add(Dense(21362, activation='softmax'))
