...
while True:
    ...
    if int(input('Choose:\n    0-Generate new model\n    1-Load previous model\n0/1: ')) == 1:
        smallModel = load('smallPhoneReviewEng')
        mediumModel = load('mediumPhoneReviewEng')
        bigModel = load('bigPhoneReviewEng')
        while True:
    ...
    smallModel.save('smallPhoneReviewEng')
    ...
    mediumModel.save('mediumPhoneReviewEng')

    bigModel = Sequential()
    bigModel.add(Embedding(input_dim=5000, output_dim=200))
    bigModel.add(GRU(256))
    ...
    bigModel.add(Dense(16, activation='relu'))
    ...
    bigModel.save('bigPhoneReviewEng')